...
Apply a strict source-boundary rule: a PRD, summary, transcript, presentation, or user-written project brief that says a policy, report, log, approval, or experiment exists is still only a `documented_claim` unless the underlying primary artifact or data was actually supplied and inspected. Precise numbers, document versions, named approvers, or phrases such as “confirmed by legal/finance” increase traceability but do not by themselves make the claim a `verified_fact`.
...
3. Build the evidence ledger using the six evidence classes above. Preserve uncertainty and conflicting values. Do not upgrade a documented or stakeholder claim merely because its source is named, quoted, versioned, or described as approved; record what artifact was actually inspected.
...
5. Read [references/five-gates.md](references/five-gates.md) and assess the gates in order: real demand; AI necessity and allocation among rules, humans, and AI; AI technical selection and feasibility validation; current-stage investment rationale and scale ROI; and operational/data/fallback readiness. Record each gate as `PASS`, `CONDITIONAL`, `FAIL`, or `NOT-ASSESSABLE`; never turn missing evidence into a failure. Judge the first gate on whether the problem is real and material, independently of whether the proposed AI solution, rollout target, or North Star definition is ready.
...
8. Design the next action. For `VALIDATE-FIRST`, specify the cheapest safe test that can change the decision and bind it to the smallest meaningful business slice: one concrete segment/category, one bounded event or operating cycle, one AI candidate node, sample or traffic limits, and exclusions. If the enterprise has not selected a value, write an explicit item to freeze before the test rather than inventing it. For `NO-GO`, state the sufficiently supported reason and, when useful, a narrower or non-AI alternative. For `GO`, state the approved scope, prerequisites, guardrails, and next review gate.
...
- Return `NO-GO` only when sufficiently supported negative evidence, a fatal constraint, an inferior build-versus-buy position, or unacceptable risk makes the current proposal not worth pursuing. “Sufficiently supported” is an evidence-sufficiency judgment and does not require labeling the underlying item `verified_fact`; a set of traceable, applicable `documented_claim` items may be sufficient. Missing information alone is not evidence for `NO-GO`.
...
- Keep `evidence_status` (`SUFFICIENT`, `PARTIAL`, or `INSUFFICIENT`) separate from the project decision. Interpret the execution-level field as evidence sufficiency for the issued decision. A `NO-GO` may be issued only with `evidence_status: SUFFICIENT` for the decisive negative evidence; describe unrelated material gaps separately instead of lowering this field to `PARTIAL`. Lower confidence when important evidence is indirect, stale, contradictory, or unverified.
